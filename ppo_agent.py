...
class InteractionState(object):
    ...
    def __init__(self, ob_space, ac_space, nsteps, gamma, venvs, stochpol, comm):
        ...
        self.buf_rews_int_ab = np.zeros((nenvs, nsteps, ac_space.n), np.float32)
...
class PpoAgent(object):
    ...
    def __init__(self, *, scope,
        ...
        with tf.variable_scope(scope):
            ...
            if self.action_balance_coef is not None:
                loss += self.stochpol.aux_loss_ab
        ...
        if self.action_balance_coef is not None:
            self._losses += [self.stochpol.aux_loss_ab, self.stochpol.feat_var_ab, self.stochpol.max_feat_ab,
                             tf.reduce_mean(self.stochpol.pd_ab.entropy()),
                             tf.reduce_mean(tf.reduce_min(self.stochpol.logits_raw_opt, axis=-1)),
                             tf.reduce_mean(tf.reduce_max(self.stochpol.logits_raw_opt, axis=-1)),
                             tf.reduce_mean(self.stochpol.logits_raw_opt)]
            self.loss_names += ["auxloss_ab", "featvar_ab", "maxfeat_ab", 'entropy_ab',
                                'logits_raw_min', 'logits_raw_max', 'logits_raw_mean']
        ...
        for step in range(num_timesteps):
            for lump in range(self.I.nlump):
                ...
                if len(all_ob) % (128 * self.I.nlump) == 0:
                    ...
                    # print("obs shape is {}".format(ob_.shape))
                    if len(ob_.shape)==4:
    ...
    def update(self):
        ...
        if self.action_balance_coef is not None:
            rew_ab_mean, rew_ab_std, rew_ab_max = self.I.buf_rews_int_ab.mean(), self.I.buf_rews_int_ab.std(), np.max(self.I.buf_rews_int_ab)
            info.update(dict(rewint_ab_mean_unnorm=rew_ab_mean,  # action balance intrinsic reward
                             rewint_ab_std_unnorm=rew_ab_std,
                             rewint_ab_max_unnorm=rew_ab_max, ))
            to_record.update({'rews_int_ab': self.I.buf_rews_int_ab})
        ...
        if self.is_train_leader:
            ...
            info.update([('loss/'+n, lossdict[n]) for n in self.loss_names])
            tnow = time.time()
    ...
    def step(self):
                ...
                if 'episode' in info:
                    ...
                    try:
                        if 'visited_rooms' not in info['episode']:
                            ...
                            info_with_places['places'] = info['episode']['visited_rooms'] # only for montezuma env
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            for start in range(0, self.I.nenvs, envsperbatch):
                ...
                self.I.buf_rews_int[mbenvinds] = tf.get_default_session().run(self.stochpol.int_rew, fd)
                if self.action_balance_coef is not None:
                    self.I.buf_rews_int_ab[mbenvinds] = tf.get_default_session().run(self.stochpol.int_rew_ab, fd)
